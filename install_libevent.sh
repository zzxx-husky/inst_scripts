...
if [ ! -d ./libevent-${LIBEVENT_VERSION}/install_dir ]; then
...
if [ -z "$(cat ${INSTRC} | grep "^export LIBEVENT_ROOT=")" ]; then
  echo "export LIBEVENT_ROOT=$(pwd)/libevent-${LIBEVENT_VERSION}/install_dir" >> ${INSTRC}
  echo "export LD_LIBRARY_PATH=\${LIBEVENT_ROOT}/lib:\${LD_LIBRARY_PATH}" >> ${INSTRC}
  echo "export DYLD_LIBRARY_PATH=\${LIBEVENT_ROOT}/lib:\${DYLD_LIBRARY_PATH}" >> ${INSTRC}
